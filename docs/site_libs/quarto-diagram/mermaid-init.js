...
const _quartoMermaid = {
	...
	setSvgSize(svg) {
		const { widthInPoints, heightInPoints, explicitHeight, explicitWidth } = this.resolveSize(svg)
	...
	resolveOptions(svgEl) {
		return svgEl.parentElement.parentElement.parentElement.parentElement.dataset
	},
	...
	postProcess(svg) {
		...
		if (options.responsive && options["figWidth"] === undefined && options["figHeight"] === undefined) {
...
window.addEventListener(
	...
	async function () {
		...
		for (const el of Array.from(document.querySelectorAll("pre.mermaid-js"))) {
			...
			const { svg: output } = await mermaid.mermaidAPI.render(`mermaid-${++i}`, text, el)
			el.innerHTML = output
			if (el.dataset.label) {
		...
		for (const svgEl of Array.from(document.querySelectorAll("svg.mermaid-js"))) {
